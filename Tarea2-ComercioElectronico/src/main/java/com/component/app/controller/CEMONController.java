...
@CrossOrigin
public class CEMONController {
	
	private static final Logger LOGGER = LoggerFactory.getLogger(CEMONController.class);
	
	@PostMapping(path="/application",produces= "application/json")
	public ResponseEntity<?> applicationComponent(@Valid @RequestBody ComponentBody body, BindingResult result) {
	...
	@PostMapping(path="/database",produces= "application/json")
	...
	public ResponseEntity<?> hardwareComponent(@Valid @RequestBody ComponentBody body, BindingResult result) {
		Map<String, Object> response = new HashMap<String, Object>();
		LOGGER.info("Errores: " + result.hasErrors());
		if (result.hasErrors()) {
			List<String> errors = result.getFieldErrors()
	...
	@PostMapping(path="/network",produces= "application/json")
	public ResponseEntity<?> networkComponent(@Valid @RequestBody ComponentBody body, BindingResult result) {
		Map<String, Object> response = new HashMap<String, Object>();
		LOGGER.info("Errores: " + result.hasErrors());
		if (result.hasErrors()) {
			List<String> errors = result.getFieldErrors()
	...
	public ResponseEntity<?> routerComponent(@Valid @RequestBody ComponentBody body, BindingResult result) {
		Map<String, Object> response = new HashMap<String, Object>();
		LOGGER.info("Errores: " + result.hasErrors());
		if (result.hasErrors()) {
			List<String> errors = result.getFieldErrors()
